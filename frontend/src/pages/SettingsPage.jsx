...
import { settings, system } from '../api'
...
export default function SettingsPage() {
  ...
  useEffect(() => {
    settings.get().then(d => {
      ...
      setHasToken(!!t)
      if (d.data?.dashboard_port) setPort(d.data.dashboard_port)
    }).catch(() => {})
  }, [])
  ...
  async function handlePortSave(e) {
    ...
    if (!p || p < 1 || p > 65535) { setPortError('Enter a valid port (1–65535)'); return }
    setPortError(''); setPortSaving(true); setPortMessage('')
    try {
      const res = await system.setPort(p)
      setPortMessage(res.message || 'Port saved. Reconnect at the new port.')
    } catch (err) {
  ...
  return (
    <div className="p-6 max-w-2xl mx-auto">
      ...
      {/* Dashboard Port */}
      <div className="bg-gray-900 border border-gray-800 rounded-xl p-6 space-y-6 mt-6">
        <div className="flex items-center gap-3 pb-4 border-b border-gray-800">
          <div className="w-9 h-9 rounded-lg bg-gray-800 flex items-center justify-center">
            <svg className="w-5 h-5 text-gray-300" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M5 12h14M12 5l7 7-7 7" /></svg>
          </div>
          <div>
            <p className="text-sm font-semibold text-white">Dashboard Port</p>
            <p className="text-xs text-gray-500">The port AutoRun listens on. The server will restart when changed.</p>
          </div>
        </div>

        {portError && <div className="bg-red-950 border border-red-800 text-red-300 text-sm px-4 py-3 rounded-lg">{portError}</div>}
        {portMessage && <div className="bg-green-950 border border-green-800 text-green-300 text-sm px-4 py-3 rounded-lg">{portMessage}</div>}

        <form onSubmit={handlePortSave} className="space-y-4">
          <div>
            ...
            <input
              ...
              placeholder="8080"
              className="w-32 bg-gray-800 border border-gray-700 text-white rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500 focus:border-transparent placeholder-gray-600 transition"
            />
          </div>
          <button type="submit" disabled={portSaving}
            className="px-4 py-2 text-sm text-white bg-brand-600 hover:bg-brand-700 disabled:opacity-50 rounded-lg transition flex items-center gap-2">
            {portSaving && <span className="w-3.5 h-3.5 border-2 border-white border-t-transparent rounded-full animate-spin" />}
            Save &amp; Restart
          </button>
        </form>
      </div>
